...
function reporte_agrupacion_producto() {
    try {
        ...
        $('#tablaDatos').empty(); // empty() en caso de que las columnas cambien
...
function reporte_conteo_detalle() {
    try {
        console.log('Consultando reporte a detalle...');
        var datos_tabla = northwind.queryAll('registros');
        // alert('1: ' + JSON.stringify(datos_tabla));
        ...
        MuestraLoader('');
        datos_tabla = alasql('SELECT cantidad, codigo_1, codigo_2, codigo_3, descripcion, precio_venta, cantidad*precio_venta AS importe, lote, fecha_caducidad, n_serie, nombre_almacen, ubicacion, nombre_usuario, latitud, longitud, fecha_captura, hora_captura, unidad_medida, n_conteo, factor, total_factor, forzado FROM ?', [datos_tabla]);
        ...
        tabla.columns.adjust().draw(true);
        ...
        OcultaLoader();
        alert('Error reporte_conteo_detalle: ' + err.message);
    }
}

function ReporteInventarioGNC() {
    try {
        ...
        MuestraLoader('');
        datos_tabla = alasql('SELECT SUM(cantidad*100)/100 AS cantidad, codigo_1, nombre_almacen FROM ? GROUP BY codigo_1, nombre_almacen', [datos_tabla]);
        ...
        tabla = $('#tablaDatos').DataTable({
            ...
            data: datos_tabla,
            columns: [
                { title: 'Tienda', data: 'nombre_almacen' },
                { title: 'UPC', data: 'codigo_1' },
                { title: 'Cantidad', data: 'cantidad' }
            ...
            createdRow: function (row, data, index) {
                //personalizamos las columnas deseadas
                $('td', row).eq(2).addClass('text-danger text-right font-weight-bold bg-white');
                // $('td', row).eq(4).addClass('small');
                // $('td', row).eq(5).addClass('text-right font-weight-bold');
                // $('td', row).eq(6).addClass('text-primary text-right font-weight-bold');
            }
        ...
        OcultaLoader();
        alert('Error ReporteInventarioGNC: ' + err.message);
    }
}
...
function CalculaTotales() {
    try {
        ...
        if (tipo_reporte === '2') {
            ...
            $('#lbl_n_productos').html('Conteo total: ' + n_productos.toLocaleString());
            ...
            //reporte a detalle
            var n_productos = tabla.column(2).data().sum();
            $('#lbl_n_productos').html('Conteo total: <b>' + n_productos.toLocaleString() + '</b>');
            $('#lbl_n_registros').html('Registros:  <b>' + n_filas + '</b>');
            $('#lbl_importe_inventario').html('');
        }
        else {
            //reportes agrupados por producto y ubicación
            var n_productos = tabla.column(0).data().sum();
            var importe = tabla.column(5).data().sum();
            $('#lbl_n_productos').html('Conteo total: ' + n_productos.toLocaleString());
            $('#lbl_n_registros').html('Registros:  <strong>' + n_filas + '</strong>');
            $('#lbl_importe_inventario').html('Valor de inventario: <strong>' + importe.toLocaleString() + ' </strong>');
        }
        ...
        alert('Error CalculaTotales: ' + err.message);
    }
...
function exporta_datos_csv(n) { //1: WA, 2: Mail, 3: CSV
    try {
        ...
        var nFilas = tabla.rows().count();

        var tipo_reporte = $('#combo_tipo_reporte').val();
        if (tipo_reporte === '0') {
            archivo_nombre = 'ConteoPorProducto ' + strFecha(0) + 'at' + strHora().replace(/:/g, '.') + '.csv';
            //cabeceras de las columnas del archivo de salida
            datos = 'Cantidad, Codigo1, Codigo2, Descripcion, PrecioVenta, Importe\n';
            //datos de la tabla para el archivo de salida
            tabla.rows().every(function (rowIdx, tableLoop, rowLoop) {
                var datos_tabla = this.data();
                console.log(JSON.stringify(datos_tabla));

                // Concatenar los valores de las columnas a 'datos'
                datos += '"' + (datos_tabla.cantidad || "").toString() + '"' +
                    ',"' + (datos_tabla.codigo_1 || "").toString() + '"' +
                    ',"' + (datos_tabla.codigo_2 || "").toString() + '"' +
                    ',"' + (datos_tabla.descripcion || "").toString() + '"' +
                    ',"' + (datos_tabla.precio_venta || "").toString() + '"' +
                    ',"' + (datos_tabla.importe || "").toString() + '"' +
                    '\n';


                // datos += datos_tabla.cantidad +
                //     ',' + datos_tabla.codigo_1 || '' +
                //     ',' + datos_tabla.codigo_2 || '' +
                //     ',' + datos_tabla.descripcion || '' +
                //     ',' + datos_tabla.precio_venta || 0 +
                //     ',' + datos_tabla.importe || 0 +
                //     '\n';
            });
            datos += '\n\n' + conteo_total + '\n' + importe_inventario + '\n' + nRegistros;
            console.log(datos);
            ...
            datos = 'Cantidad, Codigo1, Codigo2, Descripcion, PrecioVenta, Importe,  NombreAlmacen, Ubicacion\n';
            //datos de la tabla para el archivo de salida
            tabla.rows().every(function (rowIdx, tableLoop, rowLoop) {
                var datos_tabla = this.data();
                console.log(JSON.stringify(datos_tabla));
                ...
                datos += '"' + (datos_tabla.cantidad || "").toString() + '"' +
                    ...
                    ',"' + (datos_tabla.precio_venta || "").toString() + '"' +
                    ',"' + (datos_tabla.importe || "").toString() + '"' +
                    ',"' + (datos_tabla.nombre_almacen || "").toString() + '"' +
                    ',"' + (datos_tabla.ubicacion || "").toString() + '"' +
                    '\n';
            ...
            //cabeceras de las columnas del archivo de salida
            datos = 'Cantidad,Codigo1,Codigo2,Descripcion,Conteo,PrecioVenta,Importe,UnidadMedida,Lote,NSerie,FechaCaducidad,NombreAlmacen,Ubicacion,NombreUsuario,Latitud,Longitud,FechaCaptura,HoraCaptura,Factor,TotalFactor,Forzado\n';
            //datos_tabla = northwind.queryAll('registros');
            ...
            tabla.rows().every(function (rowIdx, tableLoop, rowLoop) {
                ...
                datos += '"' + (datos_tabla.cantidad || "").toString() + '"' +
                    ...
                    ',"' + (datos_tabla.codigo_2 || "").toString() + '"' +
                    ',"' + (datos_tabla.descripcion || "").toString() + '"' +
                    ',"' + (datos_tabla.n_conteo || "").toString() + '"' +
                    ',"' + (datos_tabla.precio_venta || "").toString() + '"' +
                    ',"' + (datos_tabla.importe || "").toString() + '"' +
                    ',"' + (datos_tabla.unidad_medida || "").toString() + '"' +
                    ',"' + (datos_tabla.lote || "").toString() + '"' +
                    ',"' + (datos_tabla.n_serie || "").toString() + '"' +
                    ',"' + (datos_tabla.fecha_caducidad || "").toString() + '"' +
                    ',"' + (datos_tabla.nombre_almacen || "").toString() + '"' +
                    ',"' + (datos_tabla.ubicacion || "").toString() + '"' +
                    ',"' + (datos_tabla.nombre_usuario || "").toString() + '"' +
                    ',"' + (datos_tabla.latitud || "").toString() + '"' +
                    ',"' + (datos_tabla.longitud || "").toString() + '"' +
                    ',"' + (datos_tabla.fecha_captura || "").toString() + '"' +
                    ',"' + (datos_tabla.hora_captura || "").toString() + '"' +
                    ',"' + (datos_tabla.factor || "").toString() + '"' +
                    ',"' + (datos_tabla.total_factor || "").toString() + '"' +
                    ',"' + (datos_tabla.forzado || "").toString() + '"' +
                    '\n';
            ...
            //cabeceras de la tabla para el archivo de salida
            datos = 'Codigo1, Codigo2, Descripcion, PrecioVenta, CantidadReal, CantidadTeorica, DiferenciaCantidad, ImporteReal, ImporteTeorico, DiferenciaImporte\n';
            //datos de la tabla para el archivo de salida
            tabla.rows().every(function (rowIdx, tableLoop, rowLoop) {
                var datos_tabla = this.data();
                console.log(JSON.stringify(datos_tabla));
                ...
                // Concatenar los valores de las columnas a 'datos'
                datos += '"' + (datos_tabla.codigo_1 || "").toString() + '"' +
                    ',"' + (datos_tabla.codigo_2 || "").toString() + '"' +
                    ',"' + (datos_tabla.descripcion || "").toString() + '"' +
                    ',"' + (datos_tabla.precio_venta || "").toString() + '"' +
                    ',"' + (datos_tabla.cantidad_real || "").toString() + '"' +
                    ',"' + (datos_tabla.cantidad_teorica || "").toString() + '"' +
                    ',"' + (datos_tabla.diferencia_cantidad || "").toString() + '"' +
                    ',"' + (datos_tabla.importe_real || "").toString() + '"' +
                    ',"' + (datos_tabla.importe_teorico || "").toString() + '"' +
                    ',"' + (datos_tabla.diferencia_importe || "").toString() + '"' +
                    '\n';


                // datos += datos_tabla.codigo_1 || '' +
                //     ',' + datos_tabla.codigo_2 || '' +
                //     ',' + datos_tabla.descripcion || '' +
                //     ',' + datos_tabla.precio_venta +
            ...
            archivo_nombre = 'Reporte GNC  ' + strFecha(0) + 'at' + strHora().replace(/:/g, '.') + '.txt';
            //cabeceras de las columnas del archivo de salida
            // datos = 'Tienda,Codigo1,Cantidad\n';
            ...
            tabla.rows().every(function (rowIdx, tableLoop, rowLoop) {
                ...
                // Concatenar los valores de las columnas a 'datos'
                datos += '"' + (datos_tabla.nombre_almacen || "").toString() + '"' +
                    ',"' + (datos_tabla.codigo_1 || "").toString() + '"' +
                    ',"' + (datos_tabla.cantidad || "").toString() + '"' +
                    '\n';

                // datos += datos_tabla.nombre_almacen || '' +
                //     '\t' + datos_tabla.codigo_1 || '' +
                ...
                try {
                    window.plugins.socialsharing.canShareViaEmail(function (e) {
                        ...
                        window.plugins.socialsharing.shareViaEmail(
                            'Te envío adjunto el reporte de inventario: ' + archivo_nombre + '<br /><br />Creado por ' + nombre_app + ' <a href="https://' + sitioWeb + '" target="_blank">' + sitioWeb + '</a>', // can contain HTML tags, but support on Android is rather limited:  http://stackoverflow.com/questions/15136480/how-to-send-html-content-with-image-through-android-default-email-client
...
$('#tablaDatos tbody').on('click', 'tr', function () {
    ...
    // var index = tabla.row(this).index();
});
