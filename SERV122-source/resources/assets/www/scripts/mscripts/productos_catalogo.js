...
function consulta_lista_0(lectura) {
    ...
    try {
        MuestraLoader('');
        var query = 'SELECT * FROM catalogos ORDER BY id_producto ASC';
        if (lectura !== '') {
        ...
        base_datos.transaction(function (ts) {
            ts.executeSql(query, [],
                function (ignored, resultSet) {
                    ...
                    console.log('Registros encontrados: ' + dt);

                    $('#lbl_total').html(dt + ' registros encontrados.');
                    if (dt < lote) {
                        lote = dt;
                    }
            ...
            OcultaLoader();
            console.error('Error 1 consulta_lista_0: ' + error.message);
            alert('Error 1 consulta_lista_0: ' + error.message);
        });
        ...
        OcultaLoader();
        console.error('Error 2 consulta_lista_0: ' + err.message);

        alert('Error 2 consulta_lista_0: ' + err.message);
    }
