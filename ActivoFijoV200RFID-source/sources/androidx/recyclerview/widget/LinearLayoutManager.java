    ...
    boolean performAccessibilityAction(int i, Bundle bundle) {
        ...
        if (i == 16908343 && bundle != null) {
            if (this.mOrientation == 1) {
                ...
                min = Math.min(i2, getRowCountForAccessibility(this.mRecyclerView.mRecycler, this.mRecyclerView.mState) - 1);
            } else {
                int i3 = bundle.getInt(AccessibilityNodeInfoCompat.ACTION_ARGUMENT_COLUMN_INT, -1);
                if (i3 < 0) {
                    return false;
                }
                min = Math.min(i3, getColumnCountForAccessibility(this.mRecyclerView.mRecycler, this.mRecyclerView.mState) - 1);
            }
            if (min >= 0) {
