...
class MassIndexDataAdapter extends XYSeriesDataAdapterBase {
    int _movingAveragePointsCount;
    int _sumPointsCount;
    ...
    public static MassIndexDataAdapter create(CalculatedSeriesDataWrapper calculatedSeriesDataWrapper, long j, Object obj, int i, int i2) {
        MassIndexDataAdapter massIndexDataAdapter = new MassIndexDataAdapter(calculatedSeriesDataWrapper, i, i2);
        massIndexDataAdapter.initialize(j);
        massIndexDataAdapter.setSyncObject(obj);
        return massIndexDataAdapter;
    }

    MassIndexDataAdapter(CalculatedSeriesDataWrapper calculatedSeriesDataWrapper, int i, int i2) {
        super(calculatedSeriesDataWrapper);
        this._movingAveragePointsCount = i;
        this._sumPointsCount = i2;
    }

    @Override // com.devexpress.dxcharts.SeriesDataAdapterBase
    long createNativeAdapter(long j) {
        return nativeSetData(getData(), j, this._movingAveragePointsCount, this._sumPointsCount);
    }
