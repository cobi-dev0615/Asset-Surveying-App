...
public class HkdfStreamingPrf implements StreamingPrf {
    ...
    public static StreamingPrf create(HkdfPrfKey key) throws GeneralSecurityException {
        Bytes salt = key.getParameters().getSalt();
        return new HkdfStreamingPrf(HASH_TYPE_CONVERTER.toProtoEnum(key.getParameters().getHashType()), key.getKeyBytes().toByteArray(InsecureSecretKeyAccess.get()), salt == null ? new byte[0] : salt.toByteArray());
    }
