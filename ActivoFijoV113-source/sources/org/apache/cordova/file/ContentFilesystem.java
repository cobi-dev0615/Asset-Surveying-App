...
public class ContentFilesystem extends Filesystem {
    ...
    public Uri toNativeUri(LocalFilesystemURL inputURL) {
        String substring = inputURL.uri.getEncodedPath().substring(this.name.length() + 2);
        if (substring.length() < 2) {
            return null;
        }
        String str = "content://" + substring;
        String encodedQuery = inputURL.uri.getEncodedQuery();
        if (encodedQuery != null) {
            str = str + '?' + encodedQuery;
        }
        String encodedFragment = inputURL.uri.getEncodedFragment();
    ...
    public LocalFilesystemURL toLocalUri(Uri inputURL) {
        ...
        String encodedPath = inputURL.getEncodedPath();
        if (encodedPath.length() > 0) {
            encodedPath = encodedPath.substring(1);
        }
        Uri.Builder appendPath = new Uri.Builder().scheme(LocalFilesystemURL.FILESYSTEM_PROTOCOL).authority("localhost").path(this.name).appendPath(inputURL.getAuthority());
        if (encodedPath.length() > 0) {
            appendPath.appendEncodedPath(encodedPath);
        }
        return LocalFilesystemURL.parse(appendPath.encodedQuery(inputURL.getEncodedQuery()).encodedFragment(inputURL.getEncodedFragment()).build());
    }
    ...
    */
    ...
    public String filesystemPathForURL(LocalFilesystemURL url) {
        File mapUriToFile = this.resourceApi.mapUriToFile(toNativeUri(url));
