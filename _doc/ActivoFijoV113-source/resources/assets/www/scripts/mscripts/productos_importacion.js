...
function carga_tabla() {
    try {
        ...
        //convertimos el JSON en string para cambiar el nombre de las cabeceras por el nuevo formato
        //var strExcJson = JSON.stringify(exceljson).replace(/codigo_1|codigo_2|codigo_3|descripcion|precio_venta|cantidad_teorica/gi, function (matched) {
        //    return mapObj[matched];
        //});


        //strExcJson = strExcJson.replace("undefined", "Descripcion");

        //exceljson = JSON.parse(strExcJson);  //convertimos nuevamente el string en JSON

        exceljson = remueveDuplicadosArray(); //eliminamos los códigos duplicados de la columna Codigo1

        //alert(JSON.stringify(exceljson));
        
        ////validamos que el archivo tenga solo 4 columnas
        //var columnas = BindTableHeader(exceljson);
        ...
        ////    $('#txt_archivo').val('');
        ////    calcula_totales(0);
        ////    muestraAlerta3('El catálogo de productos debe tener el formato requerido.');
        ////    return;
        ////}
        ...
        $.each(exceljson, function (i, dato_tabla) {
            ...
            //codigo_1 = exceljson[i][columnas[0]] || '';
            //codigo_2 = exceljson[i][columnas[1]] || '';
            //codigo_3 = exceljson[i][columnas[2]] || '';
            //descripcion = exceljson[i][columnas[3]] || 'Producto sin descripción';
            //precio_venta = exceljson[i][columnas[4]] || 0;
            //cantidad_teorica = exceljson[i][columnas[5]] || 0;

            //codigo_1 = exceljson[i].Codigo1 || 'error';
            //codigo_2 = exceljson[i].Codigo2 || '';
            //codigo_3 = exceljson[i].Codigo3 || '';
            //descripcion = exceljson[i].descripcion || Codigo1;
            //precio_venta = exceljson[i].PrecioVenta || 0;
            //cantidad_teorica = exceljson[i].CantidadTeorica || 0;
        ...
        if (diferenciaTablas > 0) {
         ...
         /*
//        var columns = BindTableHeader(exceljson, tableid); //Conseguimos las cabeceras de las columnas del archivo de Excel
        ...
        base_datos.transaction(function (ts) {
            ...
            $.each(exceljson, function (i, dato_tabla) {
                ...
                ts.executeSql('INSERT INTO catalogos VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                    [id_inventario, z, codigo_1, codigo_2, codigo_3, descripcion, precio_venta, '', '', '', '', '', um, '', cantidad_teorica, 0, 0, 0, cantidad_real]);
