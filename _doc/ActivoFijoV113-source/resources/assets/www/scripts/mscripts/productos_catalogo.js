            ...
            ts.executeSql(query, [],
                function (ignored, resultSet) {
                    ...
                    for (var i = 0; i < resultSet.rows.length; i++) { 
                        ...
                        var fila = '<li class="list-group-item card mt-1 waves-effect" id="' + id_producto + '"> '
    ...
    base_datos.transaction(function (ts) {
        ...
        ts.executeSql('SELECT COUNT(id_producto) AS n_registros FROM catalogos', [],
            function (ignored, resultSet) {
                ...
                if (lectura === '') {
                    mensaje = 'Mostrando primeros ' + n + ' productos de ' + n_productos;
                }
...
function eliminar_producto(id_seleccionado) {
    ...
    try {
        ...
        //northwind.deleteRows('catalogos', { ID: id_producto });
        //northwind.commit();

        base_datos.transaction(function (ts) {
            ts.executeSql('DELETE FROM catalogos WHERE id_producto=' + id_seleccionado);
        }, function (error) {
        ...
        base_datos.transaction(function (ts) {
            ts.executeSql(query, [],
                function (ignored, resultSet) {
                    ...
                    precioVentaEtiqueta = Number(resultSet.rows.item(0).precio_venta).toFixed(3) || 0;
                    precioVentaEtiqueta = precioVentaEtiqueta.toLocaleString();
                    OcultaLoader();
...
$('#btnImportacionSSR').click(function(){
