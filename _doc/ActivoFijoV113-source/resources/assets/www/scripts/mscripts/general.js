//datos app
var configuracionesApp = {
    nombreSoftware: 'Inventario activo fijo',
    versionSoftware: '1.1.3',
    codigoSoftware: 'SERINVFIJOAND',
    modoDebug: false,
    empresa: 1,
    id_usuario_logueado: '0',
    origenInstalacion: 'APK',
    direccionServidor: '',
    rutaServidorLicencia: ''
};

var nombre_app = configuracionesApp.nombreSoftware;
var version_producto = configuracionesApp.versionSoftware;
var origen_instalacion = 'APK';
var id_empresa = '1';
var nombre_empresa = 'SER';
var producto = configuracionesApp.codigoSoftware;
...
var nombre_servidor = '';
var ruta_php = 'ser.codigofractal.com';

var nombre_servidor_licencia = '';
var ruta_php_licencia = '';

// if (modo_prueba === 0) {
//     nombre_servidor = 'simplestockmobile.com';
//     ruta_php = 'https://' + nombre_servidor + '/myAssets/api/web/V1.0.0/';
...
function MensajeAlerta(mensaje, status, botones, timer, funcion) {

    var duracionMensaje = 0; // duración del mensaje en milisegundos
    var iconoStatus = status; //success, error, warning, info, question
    switch (status) {
        case 'danger':
            iconoStatus = 'error';
            // duracionMensaje = 10000; //si el mensaje tiene status danger el tiempo del mensaje será mayor
            break;
    }
    if (timer) {
        duracionMensaje = 5000; //si el mensaje tiene status danger el tiempo del mensaje será mayor
    }
    //opciones del mensaje a mostrar
    var opciones = {
        title: 'Atención',
        html: mensaje,
        icon: iconoStatus,
        ...
        timer: duracionMensaje,
        timerProgressBar: true,
        showConfirmButton: true,
        confirmButtonText: botones[0] || 'Aceptar',
        confirmButtonColor: '#4285F4',
        showCancelButton: botones.length > 1 ? true : false,
        cancelButtonText: botones[1] || 'Cancelar',
        cancelButtonColor: '#ffbb33'

    };
    // if (timer) {
    //     opciones.timer = duracionMensaje;
    // }
    ...
    Swal.fire(
        ...
        if (configuracionesApp.modoDebug) {
            console.log('Clic: ' + JSON.stringify(result));
        }
        if (result.value == true && funcion !== null) {
            funcion(); //si se da clic el botón Aceptar se ejecuta la función recibida
        }
    })
...
function muestraAlerta3(mensaje, status) {
    try {
        console.log(status + ': ' + mensaje);
        var duracion = 'long';
        var color_fondo = '#00B74A';
        ...
        else if (status === 'warning_long') {

            duracion = 20000;
            color_fondo = '#FF8800';
        }
        else if (status === 'success_s') {
            duracion = 'short';
            color_fondo = '#00B74A';
        }
        else if (status === 'success_l') {
            duracion = 20000;
            color_fondo = '#00B74A';
        }
...
function creaDirectoriosExtras(archivo_nombre, datos, modo_exportacion) {
    var folderArchivos = 'Reportes exportados';
    var folderApp = nombre_app;
    if (modo_exportacion === 1 || modo_exportacion === 2) {
