    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long currentTime) {
            ...
            if (j < 0 || currentTime < j) {
                return AutoScrollHelper.constrain((currentTime - r0) / this.mRampUpDuration, 0.0f, AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY) * 0.5f;
            }
            float f = this.mStopValue;
            return (AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY - f) + (f * AutoScrollHelper.constrain((currentTime - j) / this.mEffectiveRampDown, 0.0f, AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY));
        }
