...
function consulta_permisos_uso() {
    try {
        //si hacemos pruebas en el navegador no validamos este dato
...
function inicia_prueba() {
    try {
        ...
        actualiza_licencia(0, 'Demostración', strFecha(0), strFecha(7), codigo_activacion, llave_activacion, lista_negra);
        ...
        $.ajax({
            ...
            success: function (respuesta) {
                ...
                if (id_mensaje === '4' || id_mensaje === '5') {
                    ...
                    actualiza_licencia(0, '1', strFecha(0), strFecha(7), codigo_activacion, llave_activacion, lista_negra);
                }
            ...
            error: function (respuesta) {
                ...
                vibraAlerta(1000);
                //alert(respuesta);
                //alert('No pude acceder al servidor en estos momentos, intente nuevamente y verifíque que el dispositivo tenga conexión a internet. Si el problema persiste comuniquese con el administrador.');
            }
        });
    }
    catch (err) {
    ...
    try {
        ...
        $.ajax({
            ...
            success: function (respuesta) {
                ...
                // actualizaFechaSesion(strFecha(0)); //actualizamos la fecha de la última sesión para evitar errores y perdida de información del cliente tramposo

                suenaExito();
                alert(mensaje);
                $(location).attr('href', 'inicio.html'); //regresamos a la pantalla principal 
            },
            error: function (respuesta) {
                OcultaLoader();
                //vibraAlerta(1000);
                alert('Error de activación: ' + JSON.stringify(respuesta));
                //MuestraAlerta1('No pude acceder al servidor en estos momentos, intente nuevamente y verifíque que el dispositivo tenga conexión a internet. Si el problema persiste comuniquese con el administrador.');
            }
        });
...
function eliminaLicenciaServidor() {
    try {
        var datosLicencia = northwind.queryAll('permisos', { query: { ID: 1 } });
        // alert(JSON.stringify(datosLicencia));
        //eliminamos el registro de la licencia en el servidor
        MuestraLoader('');
        var idActivacion = datosLicencia[0].id_activacion;
        var phpArchivo = ruta_php_licencia + 'licenciaDispositivoEliminar.php';
        // alert('phpArchivo: ' + phpArchivo);
        var datos = 'idActivacion=' + idActivacion;
        // alert(datos);
        ...
        $.ajax({
            ...
            timeout: 7000 // establecenes un timeout de 7 segundos
        });

    } catch (err) {
        OcultaLoader();
        MuestraAlerta1('Error eliminaLicenciaServidor(): ' + err.message);
    }
}