...
function creaBDLocal() {
    try {
        ...
        var tablaUsuarios = ['id_usuario', 'usuario', 'nombre_usuario', 'password', 'rol', 'expiracion_sesion', 'acceso_app', 'eliminado'];
        if (!northwind.tableExists('usuarios')) {
            northwind.createTable('usuarios', tablaUsuarios);
            northwind.commit();
        }
        tablaUsuarios.forEach(function (elemento) {
            if (!northwind.columnExists('usuarios', elemento)) {
                northwind.alterTable('usuarios', [elemento]);
                northwind.commit();
            }
        });
...
function actualizaBDLocal() {
    try {
        ...
        //si la columna expiracion_sesion no existe en la tabla usuarios la creamos
        if (!northwind.columnExists('usuarios', 'expiracion_sesion')) {
            northwind.alterTable('usuarios', ['expiracion_sesion']);
            northwind.commit();
        }
...
function guarda_datos_sesion(id_usuario, usuario, nombre_usuario, rol, fecha) {
    try {
...
function n_usuarios() {
    ...
    console.log('Número de usuarios registrados: ' + n_registros);
    return n_registros;
}
...
//Guardamos datos de sesión
function GuardaDatosSesion(id_usuario, usuario, nombre_usuario_conteo, es_administrador, fecha) {
    try {
        northwind.truncate('sesionesSSM');
        //si esxite un registro con el id 1 se actualiza de lo contrario lo guarda como un nuevo registro
        northwind.insertOrUpdate('sesionesSSM', { ID: 1 }, {
            ...
            es_administrador: es_administrador,
            usuario_conteo: nombre_usuario_conteo,
            fecha_hora: fecha
        });
...
function ConsultaActualizacionApp() {
    try {
        ...
        var codigo_producto = configuracionesApp.producto;
        var version_producto = configuracionesApp.version_producto;
        ...
        $.ajax({
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                OcultaLoader();
                if (jqXHR.status === 0) {
                    reject("No se pudo conectar. Verifica tu red o la URL del servidor.");
                } else if (jqXHR.status == 404) {
                    reject("Página solicitada no encontrada [404].");
                } else if (jqXHR.status == 500) {
                    reject("Error interno del servidor [500].");
                } else if (textStatus === 'parsererror') {
                    reject("Error al analizar JSON solicitado.");
                } else if (textStatus === 'timeout') {
                    reject("Error de tiempo de espera.");
                } else if (textStatus === 'abort') {
                    reject("Petición AJAX abortada.");
                } else {
                    reject("Error desconocido: " + jqXHR.responseText);
                }
            },
            complete: function () {
                OcultaLoader();
        ...
        alert('Error consulta_producto_info: ' + err.message);
    }
}
