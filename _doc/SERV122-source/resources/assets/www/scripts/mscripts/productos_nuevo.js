...
function scan(valor) {
    try {
        var mensaje_prompt = "Capture el código principal del producto.";
        if (valor === 2) {
...
function consultaProducto() {
    try {
        console.warn('id_seleccionado: ' + id_seleccionado);
        MuestraLoader('');
        ...
        base_datos.transaction(function (ts) {
            ts.executeSql(query, [],
                function (ignored, resultSet) {
                    ...
                    var factor = resultSet.rows.item(0).factor || 1;
...
function guardaRegistro() {
    try {
        ...
        var subcategoria = '';
        var observaciones = '';
        var factor = Number($('#txtFactor').val()) || 1;
        var forzado = 1;

        if (codigo_1 === '') {
            vibraAlerta(500);
            suenaError();
            MuestraAlerta3('Capture el código 1 del producto.', 'warning');
        ...
        if (codigo_2 !== '') {
            ...
            if (codigo_3 !== '') {
                if (codigo_2 === codigo_3) {
                    vibraAlerta(500);
                    suenaError();
                    MuestraAlerta3('El SKU y 3 deben ser diferentes.', 'warning');
                    $('#txt_codigo_2').focus();
                    return;
                }
            }
        ...
        if (descripcion === '') {
            vibraAlerta(500);
            suenaError();
            MuestraAlerta3('Capture la descripción del producto.', 'warning');
        ...
        if (factor <= 0) {
            vibraAlerta(500);
            suenaError();
            MuestraAlerta3('El factor debe ser mayor a cero.', 'warning');
            $('#txtFactor').val('').focus();
            return;
        }
                ...
                function (ignored, resultSet) {
                    ...
                    base_datos.transaction(function (ts) {
                        ...
                        ts.executeSql('INSERT INTO catalogos VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                            [id_inventario, id_producto, codigo_1, codigo_2, codigo_3, descripcion, precio_venta, modelo, marca, categoria, subcategoria, 0, unidadMedida, observaciones, cantidad_teorica, 1, 0, 0, 0, factor]);
...
function editaRegistro() {
    try {
        ...
        if (descripcion === '') {
            vibraAlerta(500);
            suenaError();
            MuestraAlerta3('Capture la descripción del producto.');
            $('#txt_descripcion').val('').focus();
            return;
        }
        if (factor <= 0) {
        ...
        MuestraLoader('');
        var dt = 0;
        var query = "SELECT codigo_1 FROM catalogos WHERE codigo_1='" + codigo1 + "' AND codigo_2='" + codigo1 + "' AND codigo_3='" + codigo1 + "' AND id_producto!=" + id_seleccionado + "";
            ...
            ts.executeSql(query, [],
                function (ignored, resultSet) {
                    ...
                    if (dt > 0) {
                        suenaError();
                        OcultaLoader();
                        MuestraAlerta3('Este producto ya está registrado.', 'warning');
                        // $('#txt_codigo_1').focus();
                        return;
                    }
                    //
                    base_datos.transaction(function (ts) {
                        ...
                        ts.executeSql("UPDATE catalogos SET codigo_1='" + codigo1 + "', codigo_2='" + codigo_2 + "', codigo_3='" + codigo_3 + "', descripcion='" + descripcion + "', precio_venta='" + precio_venta + "', cantidad_teorica='" + cantidad_teorica + "', unidad_medida='" + unidadMedida + "', factor='" + factor + "' WHERE id_producto=" + id_seleccionado);
                        
                        //alert(id_producto);
                        northwind.update('registros', { codigo_1: codigo1, codigo_2: codigo_2 }, function (fila) {
