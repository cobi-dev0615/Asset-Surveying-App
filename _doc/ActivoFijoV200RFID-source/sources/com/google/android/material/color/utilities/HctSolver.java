...
public class HctSolver {
    ...
    static double[][] bisectToSegment(double d, double d2) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (nthVertex[0] >= AudioStats.AUDIO_AMPLITUDE_NONE) {
    ...
    static double inverseChromaticAdaptation(double d) {
        ...
        return MathUtils.signum(d) * Math.pow(Math.max(AudioStats.AUDIO_AMPLITUDE_NONE, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d);
    }
    ...
    static int findResultByJ(double d, double d2, double d3) {
        ...
        while (i < 5) {
            ...
            double d5 = pow;
            double pow2 = Math.pow(((d2 == AudioStats.AUDIO_AMPLITUDE_NONE || sqrt == AudioStats.AUDIO_AMPLITUDE_NONE) ? 0.0d : d2 / Math.sqrt(d4)) * pow, 1.1111111111111112d);
            double d6 = sqrt;
