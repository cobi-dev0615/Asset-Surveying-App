...
final class CctTransportBackend implements TransportBackend {
    ...
    public HttpResponse doSend(HttpRequest httpRequest) throws IOException {
        ...
        httpURLConnection.setRequestProperty("User-Agent", String.format("datatransport/%s android/", "4.0.0"));
        ...
        if (httpRequest.pseudonymousId != null) {
            httpURLConnection.setRequestProperty(COOKIE_HEADER_KEY, String.format("NID=%s", httpRequest.pseudonymousId));
        }
    ...
    private String findPseudonymousId(BackendRequest backendRequest) {
        Iterator<EventInternal> it = backendRequest.getEvents().iterator();
        if (!it.hasNext()) {
            return null;
        }
        String pseudonymousId = it.next().getPseudonymousId();
        Iterator<EventInternal> it2 = backendRequest.getEvents().iterator();
        while (it2.hasNext()) {
            String pseudonymousId2 = it2.next().getPseudonymousId();
            if (!Objects.equals(pseudonymousId, pseudonymousId2)) {
                Logging.w(LOG_TAG, "Invalid pseudonymous id event found: %s", pseudonymousId2);
                return null;
            }
        }
    ...
    public BackendResponse send(BackendRequest backendRequest) {
        ...
        try {
            HttpResponse httpResponse = (HttpResponse) Retries.retry(5, new HttpRequest(url, requestBody, r3, findPseudonymousId(backendRequest)), new Function() { // from class: com.google.android.datatransport.cct.CctTransportBackend$$ExternalSyntheticLambda0
    ...
    static final class HttpRequest {
        ...
        HttpRequest(URL url, BatchedLogRequest batchedLogRequest, String str, String str2) {
            this.url = url;
        ...
        HttpRequest withUrl(URL url) {
            return new HttpRequest(url, this.requestBody, this.apiKey, this.pseudonymousId);
        }
    }
}
