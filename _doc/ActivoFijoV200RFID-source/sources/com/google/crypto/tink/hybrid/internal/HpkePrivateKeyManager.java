...
public final class HpkePrivateKeyManager {
    private static final PrimitiveConstructor<HpkePrivateKey, HybridDecrypt> HYBRID_DECRYPT_PRIMITIVE_CONSTRUCTOR = PrimitiveConstructor.create(new ConfigurationV0$$ExternalSyntheticLambda7(), HpkePrivateKey.class, HybridDecrypt.class);
    private static final PrimitiveConstructor<HpkePublicKey, HybridEncrypt> HYBRID_ENCRYPT_PRIMITIVE_CONSTRUCTOR = PrimitiveConstructor.create(new ConfigurationV0$$ExternalSyntheticLambda6(), HpkePublicKey.class, HybridEncrypt.class);
    ...
    public static HpkePrivateKey createKey(HpkeParameters parameters, @Nullable Integer idRequirement) throws GeneralSecurityException {
        Bytes copyFrom;
        ...
        if (parameters.getKemId().equals(HpkeParameters.KemId.DHKEM_X25519_HKDF_SHA256)) {
        ...
        } else if (parameters.getKemId().equals(HpkeParameters.KemId.DHKEM_P256_HKDF_SHA256) || parameters.getKemId().equals(HpkeParameters.KemId.DHKEM_P384_HKDF_SHA384) || parameters.getKemId().equals(HpkeParameters.KemId.DHKEM_P521_HKDF_SHA512)) {
            EllipticCurves.CurveType nistHpkeKemToCurve = HpkeUtil.nistHpkeKemToCurve(parameters.getKemId());
            ...
            copyFrom2 = SecretBytes.copyFrom(BigIntegerEncoding.toBigEndianBytesOfFixedLength(((ECPrivateKey) generateKeyPair.getPrivate()).getS(), HpkeUtil.getEncodedPrivateKeyLength(parameters.getKemId())), InsecureSecretKeyAccess.get());
        } else {
            throw new GeneralSecurityException("Unknown KEM ID");
        }
        return HpkePrivateKey.create(HpkePublicKey.create(parameters, copyFrom, idRequirement), copyFrom2);
    }
